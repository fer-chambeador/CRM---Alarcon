 ...
 *
 * Abierto (sin secreto), consistente con /api/vambe/webhook. Bajo riesgo:
 * solo rellena empresa vacia de un lead que ya existe; nunca crea ni sobreescribe.
 */
...
export async function POST(req: NextRequest) {
  ...
  const { data: leads } = await supabase.from('leads').select('id, empresa').ilike('telefono', '%' + digits).limit(1)
  const lead = leads && leads[0]
  if (!lead) return NextResponse.json({ ok: true, matched: false })
  const current = clean(lead.empresa)
  if (current) return NextResponse.json({ ok: true, matched: true, changed: false, kept: current })
  await supabase.from('leads').update({ empresa }).eq('id', lead.id)
  await supabase.from('lead_actividad').insert({ lead_id: lead.id, tipo: 'empresa_capturada', descripcion: 'Empresa capturada del chat: ' + empresa, metadata: { source: 'vambe_chat' } })
