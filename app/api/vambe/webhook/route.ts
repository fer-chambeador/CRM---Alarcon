...
async function createLeadFromStage(supabase: any, aiContactId: string, newStageId: string, mappedStatus: string | undefined): Promise<any> { if (!mappedStatus || mappedStatus === 'descartado') return null; /* 7-sep-2026 (Fer): candidatos fuera del CRM. Sin formulario NO se crea lead desde etapas de puro trafico de WhatsApp (Lanzamiento, Contactados via WhatsApp, Asistencia Humana): ahi caen las personas que buscan chamba, no empleadores. Si de verdad es empresa y avanza a Interesado / Llamadas / Agendados / Ganados, entra al CRM en ese momento y no se pierde nada. */ const STAGES_TRAFICO_SIN_FORM = new Set(['05b9af0a-9bcb-4faf-a114-bdd47517a97a', '5847352c-f983-4e8b-b635-b19797d031a8', 'dd41a38e-3b22-42f3-a6d3-b130b9ca449f']); if (STAGES_TRAFICO_SIN_FORM.has(newStageId)) { console.log('[vambe] lead NO creado en CRM (trafico sin form): stage=' + newStageId + ' contact=' + aiContactId); return null; } try { const vk = process.env.VAMBE_API_KEY || ''; const stages = [newStageId, '96c42cda-2828-45db-973c-3bc63a8141fd', '05b9af0a-9bcb-4faf-a114-bdd47517a97a', '971fe009-72d1-44fb-932b-aa94adcec4db', 'cd0ab574-c844-4346-bea3-4ddd084fcb92', '2fc44415-960f-4dbd-b65b-1500636fc41a', 'c86a7911-ef9d-4f6d-8c90-3e9a9a4d6b50']; let c: any = null; for (const sid of stages) { if (!sid) continue; try { const cr = await fetch('https://api.vambe.me/api/public/contacts?days=60&stage_id=' + sid, { headers: { 'x-api-key': vk } }); const cj = await cr.json(); const carr: any[] = Array.isArray(cj) ? cj : (cj.contacts || cj.data || []); const found = carr.find(function (x: any) { return String(x.id) === String(aiContactId); }); if (found) { c = found; break; } } catch (e4) {} } const phone = c && c.phone ? String(c.phone) : ''; const digits = phone.replace(/[^0-9]/g, '').slice(-10); if (digits.length !== 10) { console.warn('Vambe stage sin telefono ' + aiContactId); return null; } const email = (c.email && String(c.email).includes('@')) ? c.email : digits + '@clientes.chambas.ai'; const exq = await supabase.from('leads').select('*').or('vambe_contact_id.eq.' + aiContactId + ',email.eq.' + email).limit(1).maybeSingle(); if (exq.data && exq.data.id) return exq.data; const insq = await supabase.from('leads').insert({ email: email, nombre: c.name || null, telefono: phone, canal_adquisicion: 'Vambe', status: mappedStatus, vambe_contact_id: aiContactId, vambe_stage_id: newStageId }).select('*').single(); if (insq.error) { console.error('Vambe stage insert error', insq.error.message); return null; } if (insq.data) { await supabase.from('lead_actividad').insert({ lead_id: insq.data.id, tipo: 'vambe_lead_created', descripcion: 'Lead creado desde Vambe al entrar a etapa (sin form)', metadata: { source: 'vambe_stage_no_form', stage_id: newStageId } }); return insq.data; } return null; } catch (e2) { console.error('Vambe stage create error', e2); return null; } }
